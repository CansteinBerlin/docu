...
1. Ausführung als Java-Spieler: `/linkaccount <"Gamer-Tag" = XBOX-Name (mit oder ohne Punkt)>` --> Man bekommt im Chat einen Verifizierungs-Schlüssel angezeigt.
...
Die Verlinkung kann wieder jederzeit mit dem Befehl `/unlinkaccount` aufgehoben werden. Nach der Befehls-Eingabe kann man den Server neu betreten, um mit dem vollständig getrennten Bedrock-Account weiterzuspielen.
